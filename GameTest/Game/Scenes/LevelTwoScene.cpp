...
{
    auto player1 = m_entityFactory->CreateGolfBallOne(FVector3(0.0f, 10.0f, -80.0f));
    auto player2 = m_entityFactory->CreateGolfBallTwo(FVector3(-5.0f, 5.5f, -38.0f));
    auto hole = m_entityFactory->CreateStaticBox(FVector3(1.5f, 4.0f, -20.0f), FVector3(0.3f, 3.0f, 0.3f),
    ...
    m_entityFactory->CreateInvisibleBoxCollider(FVector3(0.0f, 2.5f, -38.0f), FVector3(18.5f, 1.0f, 6.0f),
                                                FVector3(-0.2f, 0.0f, 0.0f), 0.2f, 0.5);
    ...
    // m_entityFactory->CreateBouncySphere(FVector3(-2.0f, 5.0f, -85.0f), 3.0f);
    ...
    m_entityFactory->CreateInvisibleBoxCollider(rampStartPosition,
                                                ...
                                                FVector3(-0.2f, 0.0f, 0.0f), // Rotation
                                                0.2f, 0.5                    // Elasticity and friction
    );
    ...
    // Calculate Sand Borders Collider Position and Extents
    FVector3 sandBordersColliderPosition(sandBordersStartPosition.x + ((totalSandColumns - 1) * grassSpacing) / 2.0f,
                                         sandBordersStartPosition.y,
                                         sandBordersStartPosition.z + ((totalSandRows - 1) * grassSpacing) / 2.0f);

    FVector3 sandBordersColliderExtents((totalSandColumns * grassSpacing) / 2.0f, // Half width
                                        2.0f,                                     // Y extent
                                        (totalSandRows * grassSpacing) / 2.0f     // Half depth
    );

    // Create a single invisible collider for all sand borders
    m_entityFactory->CreateInvisibleBoxCollider(sandBordersColliderPosition, sandBordersColliderExtents,
