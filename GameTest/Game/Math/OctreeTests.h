...
inline TestResult InsertSphereCollider()
{
    ...
    bool isDynamic = false; // Static collider

    octree.Insert(sphere, transform, entityID, isDynamic);
...
inline TestResult InsertBoxCollider()
{
    ...
    octree.Insert(box, transform, entityID, isDynamic);
...
inline TestResult DetectPotentialCollisions()
{
    ...
    bool isDynamic3 = false;

    // Insert colliders
    octree.Insert(sphere1, transform1, entityID1, isDynamic1);
    octree.Insert(sphere2, transform2, entityID2, isDynamic2);
    octree.Insert(sphere3, transform3, entityID3, isDynamic3);
...
inline TestResult SubdivideAfterCapacity()
{
    ...
    // With 3 colliders, assuming overlapping positions, expecting multiple collisions
    // Adjust this based on actual positions
    bool passed = (potentialCollisions.size() == 2); // Example expectation
...
inline TestResult MultipleCollidersInSingleNode()
{
    ...
    std::set<std::pair<unsigned int, unsigned int>> collisionSet = {
        {potentialCollisions[0].first, potentialCollisions[0].second},
        {potentialCollisions[1].first, potentialCollisions[1].second},
        {potentialCollisions[2].first, potentialCollisions[2].second},
        {potentialCollisions[3].first, potentialCollisions[3].second}};
...
{
    BoundingBox3D rootBounds(FVector3(-100.0f, -100.0f, -100.0f), FVector3(100.0f, 100.0f, 100.0f));
    Octree octree(rootBounds, 10, 10); // Assuming high capacity to avoid subdivision
...
inline TestResult NoMultipleDuplicatePairs()
{
    ...
    {
        bool isDynamic = false;
        octree.Insert(collider.sphere, collider.transform, collider.entityID, isDynamic);
        octree.Insert(collider.sphere, collider.transform, collider.entityID, isDynamic);
    }
